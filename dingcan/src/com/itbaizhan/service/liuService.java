...
public class liuService
{
	...
	//保存订单
	{
		String sql="insert into t_order(id,bianhao,shijian,zhuangtai,huifu,songhuodizhi,fukuanfangshi,jine,user_id,phone) values(?,?,?,?,?,?,?,?,?,?)";
		Object[] params={order.getId(),order.getBianhao(),order.getShijian(),order.getZhuangtai(),order.getHuifu(),order.getSonghuodizhi(),order.getFukuanfangshi(),order.getJine(),order.getUser_id(),order.getPhone()};
		DB mydb=new DB();
		mydb.doPstm(sql, params);
		mydb.closed();
	}
